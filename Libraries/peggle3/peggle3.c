...
#define PUSH(𝙺,X) do{Match* V=(X); if(V!=ᗜ) { Match* _=vector_add_dst(Ѧ,&𝙺); _->id=V->id; _->p=V->p; _->l=V->l; _->K=V->K; _->nK=V->nK; }}while(0)
Match* chain_cond(Arena* Ѧ, Rule* 𝚁, Match* 𝙺, u32 r, Match* 𝘮)
{
...
#undef PUSH
Match* makeMatches(Arena* Ѧ, Rule* 𝚁, u32 𝚁𝚗, bool trash, u32 𝘱, u32 𝘪, u32 𝚌l, u32 𝙼[𝚌l][𝚁𝚗])
{
    Rule 𝘳 = 𝚁[𝘪];
    u32 𝐜 = 𝙼[𝘱][𝘪];
    Match* 𝘮 = (Match*)arena_alloc(Ѧ,sizeof(Match));
    ...
    Ⴝ(𝘳.t) 
    {
        ...
        Ƈ RULE_F: 
        {
            ...
            𝙺  =chain_cond(Ѧ,𝚁,𝙺,𝘳.cL,makeMatches(Ѧ,𝚁,𝚁𝚗,trash,𝘱,𝘳.cL&~R_FLAG,𝚌l,𝙼));
        }
        𝼝 RULE_O: 
        {
            u32 c = 𝐜&C_ALTR ?𝘳.cR: 𝘳.cL;
            if(trash && c&R_TRSH) break;
            𝙺  =chain_cond(Ѧ,𝚁,𝙺,   c,makeMatches(Ѧ,𝚁,𝚁𝚗,trash,𝘱,   c&~R_FLAG,𝚌l,𝙼));
        }
        𝼝 RULE_A: 
        {
            ...
            𝙺  =chain_cond(Ѧ,𝚁,𝙺,𝘳.cR,makeMatches(Ѧ,𝚁,𝚁𝚗,trash,𝘗,𝘳.cR&~R_FLAG,𝚌l,𝙼));
        }
    }
...
Match* parse(Arena *Ѧ, Rule* 𝚁, u32 𝚁𝚗, ᔐ 𝚌, u32 id, bool trash)
{
    ...
    #endif
    Match* r = 𝙼[0][id]&C_FAIL ?ᗜ: makeMatches(Ѧ,𝚁,𝚁𝚗,trash,0,id,𝚌.l+1,𝙼);
    #ifdef DEBUG_PRINTS
    printf("Raw tree:\n");
    show_match(𝚁,r,𝚌,2);
    #endif
...
main() 
{
    ...
    */
    ...
    {
        { RULE_A,  1|R_NAME,  5                      },
        { RULE_O,  2|R_NAME,  3|R_NAME               },
        { RULE_A,  7|R_TRSH,  8                      },
        { RULE_O, 10       , 11                      },
        { RULE_O, 12       , 13                      },
        { RULE_Q, 14       ,  0                      },
        { RULE_A,  4|R_NAME,  1|R_NAME               },
        { RULE_S,  0       ,  0       , {(u8*)"(",1} },
        { RULE_A,  0|R_NAME,  9|R_TRSH               },
        { RULE_S,  0       ,  0       , {(u8*)")",1} },
        { RULE_S,  0       ,  0       , {(u8*)"0",1} },
        { RULE_S,  0       ,  0       , {(u8*)"1",1} },
        { RULE_S,  0       ,  0       , {(u8*)"+",1} },
    ...
    ;
    ᔐ s = (ᔐ){"(1+1)*0+1*(1*((1)+1*0))",23};
    Arena Ѧ;
    arena_init(&Ѧ);
    Match* M = parse(&Ѧ,rules,ARRSIZE(rules,Rule),s,0,ⴳ);
    show_match(rules,M,s,0);
    arena_free_all(&Ѧ);
}
